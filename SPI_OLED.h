...
public:
  ...
  void init() {
    ...
    for (uint8_t i = 0; i < sizeof(_oled_init); i++) SPI_write(pgm_read_byte(&_oled_init[i]));  // send initialization sequence
  }
  ...
  void print(signed int num) {  // print a signed value with 8-bit magnitude (-255..255)
  ...
  void println(signed int num) {  // same, in new line
  ...
  void print(uint8_t num) {  // print unsigned 8-bit number
    ...
    char buf[3];  // max 3 digits for uint8_t (0-255)
    uint8_t i = 0;
...
private:
  ...
  void write(uint8_t data) {
    ...
    const uint8_t mask = (1 << _scaleX) - 1;  // e.g., for _scaleX = 3, mask becomes 0b111 (invariant per call)
  ...
  uint8_t getFont(uint8_t colum, uint8_t row) {
    ...
    if (row > 4) return 0;
    colum -= 0x20;  // map ASCII char to font table index (space = 0x20 = index 0)
